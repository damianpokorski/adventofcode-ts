...
initialize(__filename, async (part, input) => {
  return input
    .map((line) => {
      const [constraintsRaw, letter, password] = line.replace(':', '').split(' ');
    ...
    .filter(([a, b, letter, password]) => {
      ...
      if (part == 1) {
        const requiredLEtterOccurences = password.split('').filter((x) => x == letter).length;
        return (a > requiredLEtterOccurences || b < requiredLEtterOccurences) == false;
      }
