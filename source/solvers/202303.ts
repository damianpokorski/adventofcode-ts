...
const getAdjecent = <T>(input: T[][], offsetX = 0, offsetY = 0) => {
  return directions
    .map((offset) => (input[offsetY + offset.y] ?? [])[offsetX + offset.x])
...
const part1 = (rows: string[]) => {
  ...
  rows = [
    [...new Array(rows[0].length)].map(() => '.').join(''),
        ...
        if (partNumbersAvailable == 2) {
          ...
          gearRatios.push(
            ...
            [
              ...
              ...(hasDigits.top == 2
                ? [
                    numberFinder(rows, x - 1, y - 1, { x: -1, y: 0 }, true),
                    numberFinder(rows, x + 1, y - 1, { x: 1, y: 0 })
                  ]
                : []),
...
initialize(__filename, async (part, input) => {
  return (part == 1 ? part1(input) : part2(input)).toString();
})
  .test(
    1,
    `467..114..
